...
exports.topup = function(payload, callback){
    ...
    async.waterfall([
            ...
            logger.info('search user by phoneID');
              Userquery.findUserByPhoneID(payload.phoneID,function(err,result){
                if(err){
                    ...
                    payload.message = config.messages.airtimeTopUp.replace('money',payload.amount).replace('user',result.name).replace('&',config.currency.symbol);
        ...
        function(sessionid,messages, callback) {
            console.log('Send push notification');
                sms.sendMessage(payload.to,payload.message, function(err,sms_response){
                if (err) {
                    var response = { statusCode: 1, additionalInfo: "Error to send Message" };
                    console.log('ERROR'+ response);
                    callback(null, sessionid);
                }else
                    callback(null, sessionid);
        ...
        function(balance, callback) {
            ...
            receipt.message = payload.message;
            receipt.additionalInfo = payload.additionalInfo;
            receipt.title = 'You have sent a Transfer '+ ' to ' + receipt.receiver;
            receipt.date = dateTime;
            receipt.type = config.receipt.type.AIRTIMEBUY;
            receipt.status = 'DELIVERED';
            receipt.owner = 0;
        ...
        function(balance,receipt, callback) {
            ...
            transacction.phoneID = receipt.emitter;
            Userquery.findAppID(receipt.emitter,function(err,result){
                transacction.description ='Transfered  to ' + payload.to;
                transacctionQuery.createTranssaction(transacction, function(err, result) {
                    if (err)
                        callback('ERROR', err);
                    else{
